...
import { buildReviewPrompt, MICRO_DEFENSE_QUESTION, REVIEW_SCHEMA } from "./prompt.js";
...
describe("M3 opt-in guard", () => {
  it("only runs when repo AND operator both opt in", () => {
    const on: StewardConfig = { ...DEFAULT_CONFIG, llm: { enabled: true } };
  ...
  it("actions runs the review when opted in, and skips it when the repo hasn't", async () => {
    ...
    await runActions(pool, "pull_request", prPayload, { client: cOn, llm: llmOn, config: { ...DEFAULT_CONFIG, llm: { enabled: true } } });
